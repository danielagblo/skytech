...
function TopScrollingBanner({
  ...
  return (
    <div className={className ? `${className} relative` : "relative flex bg-black text-white h-[45px] items-center overflow-hidden whitespace-nowrap border-b border-white/5 w-full z-10"}>
      ...
      {/* 3. RIGHT SIDE: Phone Number (with smooth fading left edge) */}
      <div className="absolute right-0 top-0 h-full flex items-center pr-4 pl-14 bg-gradient-to-l from-black via-black/95 to-transparent z-20">
        <a
          href={`tel:${contactPhone}`}
          className="text-white hover:text-brand-300 font-semibold text-xs md:text-sm flex items-center gap-1.5 transition-colors"
        >
          <svg className="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24">
