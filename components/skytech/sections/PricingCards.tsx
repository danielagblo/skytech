        ...
        return (
          <div
            ...
            className={`
              ...
              transition-all duration-300 min-w-[18.75rem]
              ${isSelected ? "bg-[#1E5AC8] text-white scale-[1.05]" : "border border-gray-400"}
            `}
          >
            {isRecommended && (
              <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                <span
                  className={`font-medium uppercase px-4 py-3 rounded-2xl border ${
                    isSelected ? "bg-white text-gray-800 border-[#1E5AC8]" : "bg-white text-gray-700 border-gray-400"
                  }`}
                >
                  Recommended
                </span>
            ...
            <div>
              ...
              <div className={`border-l-4 pl-3 mb-5 ${isSelected ? "border-white" : "border-[#1E5AC8]"}`}>
                <p className={`text-sm ${isSelected ? "text-white/90" : "text-gray-600"}`}>{plan.audience}</p>
                {plan.timeline && (
              ...
              <ul className="space-y-2">
                {plan.features.map((feature, i) => (
                  <li key={i} className="flex items-start gap-2">
                    ...
                    <span className={isSelected ? "text-white/90" : "text-gray-700"}>{feature}</span>
                  </li>
                ))}
              </ul>
            </div>
